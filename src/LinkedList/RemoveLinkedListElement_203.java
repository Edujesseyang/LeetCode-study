...
0 <= val <= 50
    */
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nAfter remove:");
        ListNode<Integer> afterRemove = new ListNode<>();
        afterRemove = removeElement(test, 1);
    ...
    /**
     * 一个linkedList经典的整体删除, 这个解法时间复杂度是O(n), 空间是O(1)
     * 这里的难点是在如果处理删除这个逻辑, 有很多方法, 比如建立一个prev, 让current在prev的前面找相同的, 找到了就把prev link 到current的下一个.
     * 最后在检查一个head是不是相同, 如果是的话head进一位.
     * 这里的做法是先在head的前面加上一个dummy, 用dummy作为头, 用current的下一位来比对, 这时就能覆盖到head了.
     * 最后return dummy的下一位.
     */
